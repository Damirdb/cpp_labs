...
#include <iomanip>
#include <cstring>
// добавить структуру где уточняетс вход платный или нет, если да то можно указать цену, если нет то не запоняется /* Возвращает продолжительность работы в минутах
    int duration(const Time& closeTime) const {
        return (closeTime.hours * 60 + closeTime.minutes) - (hours * 60 + minutes);
    }  не работает проверка, нужна разница от начала и конца */
